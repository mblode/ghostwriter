...
- [ghostwriter](./skills/ghostwriter/SKILL.md) drafts, rewrites, and reviews from a private per-platform profile, strips the machine tells that mark prose as AI, and applies a communication-strategy layer to high-stakes messages.
- [train-ghostwriter](./skills/train-ghostwriter/SKILL.md) builds those profiles from your own writing samples, with a held-out split kept clean for evaluation.
- [evaluate-ghostwriter](./skills/evaluate-ghostwriter/SKILL.md) runs blind baseline-versus-profile comparisons so you can check whether a profile actually helps.
...
> Use ghostwriter to draft a Slack reply saying I agree with the direction and can review the PR tomorrow morning.
...
- **By hand.** Edit `~/.config/ghostwriter/soul.md` (your cross-platform core: openers, sign-off, spelling, fingerprint words) and `~/.config/ghostwriter/slack.md` (the register for one platform). Add `email.md`, `linkedin.md`, or any `<platform>.md` you need. `soul.md` is optional; a platform file is required to draft for that platform.
- **From real writing.** Point `train-ghostwriter` at samples you have actually written, and it derives the profiles for you (see below).
...
> Use train-ghostwriter to build a Slack profile from these local writing samples.
...
> Use evaluate-ghostwriter to compare the Slack profile against its held-out cases using Codex.
...
Personal data lives outside this repository, under `GHOSTWRITER_HOME` (default `~/.config/ghostwriter`):
